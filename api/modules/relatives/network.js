...
router.get('/:id', (req, resp) => {
    controller.listRelatives(req.params.id).then((data) => {
...
router.put('/', (req, resp) => {
    controller.updateRelative(req.body).then((data) => {
        response.Sucess(req, resp, data, 200);
    }).catch((error) => {
        response.Error(req, resp, error.message, 500);
    })
...
router.delete('/', (req, resp) => {
    controller.deleteRelatives().then((result) => {
        response.Sucess(req, resp, result, 200);
    }).catch((err) => {
        response.Error(req, resp, err.message, 500);
    })
...
router.post('/', (req, resp) => {
    controller.insertRelative(req.body).then((result) => {
        response.Sucess(req, resp, result, 200);
    }).catch((error) => {
        response.Error(req, resp, error.message, 500);
    })
